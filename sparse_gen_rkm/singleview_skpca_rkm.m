function [V, D, K] =  singleview_skpca_rkm(X, ft_map, params, s, no_sparse, on_stm, method)
    % ****************************************************************
    % X: data matrix
    ...
    if (no_sparse == 0) && (strcmp(method, 'gpowerl0') == 1)
    ...
    Xg = V*sqrt(abs(diag(D)))*V'; % can be replaced by Cholesky decomposition with adding small noise to ensure positive definiteness
    
    if strcmp(method, 'gpowerl0') && on_stm == 0
        % sparse PCA - Gpower method l0 norm, Euclidean space
        gamma = params{6} / max(sum(K.^2, 1));
        gamma = gamma*ones(1, s); 
        V = GPower(Xg, gamma.^2, s, 'l0', 0);
        D = diag(V'*K*V);
        
    elseif strcmp(method, 'gpowerl0') && on_stm == 1
        % sparse PCA - Gpower method l0 norm, Stiefel manifold
        gamma = params{6} / max(sum(K.^2, 1));
        gamma = gamma*ones(1, s); 
        mu = (1:s).^(-1); 
        V = GPower(Xg, gamma.^2, s, 'l0', 1, mu);
        D = diag(V'*K*V);
